...
sql = "SELECT symbol_list.symbol, symbol_list.uid, symbol_list.fsym, symbol_list.tsym FROM symbol_list INNER JOIN instruments ON symbol_list.symbol = instruments.symbol WHERE symbol_list.symbol <> '"+ get_portf_suffix() +"' instruments.asset_class = 'CR:' AND disabled =0"
cr.execute(sql)
rs = cr.fetchall()
i = 1
for row in rs:
