...
def get_market_name(m):
    ...
    try:
        connection = pymysql.connect(host=db_srv,user=db_usr,password=db_pwd, db=db_name,charset='utf8mb4',cursorclass=pymysql.cursors.DictCursor)
        cr = connection.cursor(pymysql.cursors.SSCursor)
        sql = "SELECT market_label FROM market WHERE market_id='"+ str(ac) +"' "
        cr.execute(sql)
        rs = cr.fetchall()
        for row in rs: r = row[0]
    except Exception as e: print(e)
...
def get_unit(m):
    ...
    try:
        connection = pymysql.connect(host=db_srv,user=db_usr,password=db_pwd, db=db_name,charset='utf8mb4',cursorclass=pymysql.cursors.DictCursor)
        cr = connection.cursor(pymysql.cursors.SSCursor)
        sql = "SELECT currency_code FROM market WHERE market_id = '"+ str(m) +"' "
        cr.execute(sql)
        rs = cr.fetchall()
        for row in rs: r = row[0]
    except Exception as e: print(e)
...
def gen_portf():
    try:
        ...
        connection = pymysql.connect(host=db_srv,user=db_usr,password=db_pwd, db=db_name,charset='utf8mb4',cursorclass=pymysql.cursors.DictCursor)
        cr = connection.cursor(pymysql.cursors.SSCursor)
        sql = "SELECT asset_class_id WHERE asset_class_id <> 'PF:' AND asset_class_id <> 'MA:' AND asset_class_id <>'BD:' AND asset_class_id <>'CO:' "
        cr.execute(sql)
        rs = cr.fetchall()
        for row in rs:
